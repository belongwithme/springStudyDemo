...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
		    ...
		    //调用容器准备刷新的方法，获取容器的当时时间，同时给容器设置同步标识.刷新前的预处理。
			prepareRefresh();
			...
			try {
				...
				//对于注解容器，ConfigurationClassPostProcessor
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		refreshBeanFactory();//刷新当前Application的BeanFactory
		return getBeanFactory(); //方法获取BeanFactory并返回
	}
	 ...
	 */
	/**
	 * 向给定的可配置可列举的 Bean 工厂注册 BeanPostProcessor
	 * @param beanFactory
	 */
	...
	protected void finishRefresh() {
		// 清除上下文级别的资源缓存，例如扫描产生的 ASM 元数据。
		clearResourceCaches();

		// 初始化用于管理生命周期的处理器，用于控制上下文中 bean 的生命周期事件。
		initLifecycleProcessor();

		// 调用生命周期处理器的 onRefresh() 方法，通知 bean 的生命周期已经刷新。
		getLifecycleProcessor().onRefresh();

		// 发布一个 ContextRefreshedEvent 事件，通知监听器应用程序上下文已经完成刷新。
		publishEvent(new ContextRefreshedEvent(this));

		// 如果启用了 LiveBeansView MBean 功能，将当前应用程序上下文注册到 MBean 中，以便监控 bean 的状态。
