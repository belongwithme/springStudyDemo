...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	/**
	 * Spring的环境（environment）配置中用于合并父环境（parent）和当前环境的属性。
	 * 环境是Spring应用程序的一部分，
	 * 用于配置应用程序中的各种参数，比如数据库连接信息、日志级别、Spring配置文件等。
	 * ConfigurableEnvironment接口定义了对环境配置的修改和访问方法。
	 * @param parent the environment to merge with
	 */
	@Override
	public void merge(ConfigurableEnvironment parent) {
		/**
		 * 历父环境的所有属性源（propertySources），将父环境中的属性源添加到当前环境中的属性源列表中，
		 * 如果当前环境中已经包含同名的属性源，则不会添加。
		 * 这里的属性源是指一个键值对集合，用于存储应用程序的配置信息，
		 * 比如系统属性、环境变量、配置文件等。
		 */
		...
		/**
		 * 将父环境的活动配置文件（activeProfiles）合并到当前环境的活动配置文件中。
		 * 活动配置文件是指在应用程序运行时激活的配置文件，
		 * 可以用来根据环境来加载不同的配置。
		 * 如果当前环境中已经包含父环境的活动配置文件，则不会添加。
		 * 这里使用了同步代码块来确保线程安全。
		 */
		...
		/**
		 * 将父环境的默认配置文件（defaultProfiles）合并到当前环境的默认配置文件中。
		 * 默认配置文件是指在应用程序启动时自动加载的配置文件，
		 * 如果当前环境中已经包含了保留的默认配置文件（RESERVED_DEFAULT_PROFILE_NAME），
		 * 则会先删除再添加。同样使用了同步代码块来确保线程安全。
		 */
